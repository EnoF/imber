//grunt-start
/// <reference path="bower_components/type-def/angularjs/angular.d.ts" />
/// <reference path="bower_components/type-def/node/node.d.ts" />
/// <reference path="bower_components/type-def/mocha/mocha.d.ts" />
/// <reference path="bower_components/type-def/sinon-chai/sinon-chai.d.ts" />
/// <reference path="../test/angular-mocks.d.ts" />
/// <reference path="templates.ts" />
/// <reference path="../test/context.ts" />
/// <reference path="../test/definitions.ts" />
/// <reference path="../test/globals.ts" />
/// <reference path="core/models/errors.ts" />
/// <reference path="core/models/serializable.ts" />
/// <reference path="core/models/session.ts" />
/// <reference path="core/models/user.ts" />
/// <reference path="core/dao/dao.ts" />
/// <reference path="core/dao/userDAO.ts" />
/// <reference path="widgets/login/src/loginForm.ts" />
/// <reference path="widgets/login/src/loginVM.ts" />
/// <reference path="widgets/login/login.ts" />
/// <reference path="widgets/login/test/unit/login.step.ts" />
/// <reference path="app.ts" />
//grunt-end
